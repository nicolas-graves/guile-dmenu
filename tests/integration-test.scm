...
(define (start-swing-route-client session scenario)
  (let* ((output (pipe))
         (errors (pipe))
         (pid (primitive-fork)))
    (if (zero? pid)
        (begin
          ...
          (execlp "guile" "guile" "--no-auto-compile"
                  ...
                  "-s" (string-append project-dir "/tests/swing-route-client.scm")
                  scenario))
        (begin
          (close-port (cdr output))
          (close-port (cdr errors))
  ...
  (for-each
   (lambda (case)
     (let* ((scenario (car case))
            ...
            (expected (caddr case))
            (event-count (length (read-river-manager-events session)))
            (client (start-swing-route-client session scenario)))
       (eventually (string-append "Swing route fixture configured: " scenario)
                   (lambda ()
                     (configured-event
                      (drop (read-river-manager-events session) event-count))))
       ...
       (let ((status (cdr (waitpid (car client))))
             (stdout (get-string-all (cadr client)))
             (stderr (get-string-all (caddr client))))
         (test-equal (string-append "Swing route fixture exits: " scenario)
           0 (status:exit-val status))
         (test-equal (string-append "run-wayland-swing routes workflow: " scenario)
           expected stdout)
         (test-equal (string-append "Swing route fixture diagnostics: " scenario)
           "" stderr))))
